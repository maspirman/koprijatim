...
class FrontendController extends Controller
{
    ...
    public function index()
    {
        ...
        $upcoming_events = Events::select('title','date','content','time','cost','image','slug','category_id','venue_location','id')->where('date', '>', $tgl)->where(['status' => 'publish'])->get();
...
//        }
    ...
    public function user_campaign(){
     ...
     public function user_letter(){
        if (Auth::guard('web')->check()){
            return redirect()->route('user.letter.new');
        }
        return view('frontend.user.login')->with(['title' => __('Silahkan login untuk submit surat')]);
    }
