...
//    change password after login
    ...
    public function followed_user_campaigns_list($id)
    {
        ...
        foreach ($user_follow_data as $data){
            $all_following_user_campaigns = Cause::select('id','title','amount','raised','image')->where('user_id',$data->campaign_owner_id)->orWhere('admin_id',$data->campaign_owner_id)->paginate(20)->withQueryString();
            $filter_data = $all_following_user_campaigns->transform(function($item){
                $image_url = null;
                if(!empty($item->image)){
