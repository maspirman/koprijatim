...
class HomeController extends Controller
{
    ...
    {
        $all_donation_category = CauseCategory::select('id','title','image')->where(['status' => 'publish'])->whereDate("deadline", '>', \Carbon\Carbon::now())->paginate(10)->withQueryString();
        ...
        $feature_cause = Cause::select('id','title','raised','amount','image')->where(['status' => 'publish','featured' => 'on'])->whereDate("deadline", '>', \Carbon\Carbon::now())->paginate(20)->withQueryString();
        ...
        $all_recent_donation = Cause::select('id','title','amount','raised','image')->where(['status' => 'publish'])->whereDate("deadline", '>', \Carbon\Carbon::now())->orderBY('id','desc')->paginate(20)->withQueryString();
    ...
    {
        $all_donations = Cause::select('id','title','amount','raised','image')->where(['categories_id' =>$id, 'status' => 'publish'])->paginate(10)->withQueryString();
